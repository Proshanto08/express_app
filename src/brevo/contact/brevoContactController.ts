...
export const createContactController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.status === 201) {
      res.status(201).json({ message: 'Contact successfully created', data: result.data });
...
export const getContactController = async (req: Request, res: Response) => {
  ...
  if (result.status === 200) {
    res.status(200).json({ message: 'Contact retrieved successfully', data: result.data });
  } else if (result.status === 400) {
    res.status(400).json({ error: 'Bad request', message: result.message });
  } else {
    res.status(result.status).json({ error: 'Internal server error', message: result.message });
...
export const updateContactController = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (result.status === 200) {
      res.status(200).json({ message: 'Contact successfully updated', data: result.data });
    } else if (result.status === 400) {
      res.status(400).json({ error: 'Bad request', message: result.message });
    } else {
      res.status(result.status).json({ error: 'Internal server error', message: result.message });
...
export const getAllContactsController = async (_req: Request, res: Response) => {
  ...
  if (result.status === 200) {
    res.status(200).json({ message: 'Contacts retrieved successfully', data: result.data });
  } else if (result.status === 400) {
    res.status(400).json({ error: 'Bad request', message: result.message });
  } else {
    res.status(result.status).json({ error: 'Internal server error', message: result.message });
