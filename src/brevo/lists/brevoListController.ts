...
export const createListController = async (req: Request, res: Response) => {
  ...
  if (result.status === 201) {
    res.status(201).json({ message: 'List successfully created', data: result.data });
...
export const getListsController = async (_req: Request, res: Response) => {
...
export const getListByIdController = async (req: Request, res: Response) => {
  ...
  if (result.status === 200) {
    res.status(200).json({ message: 'List details retrieved successfully', data: result.data });
  } else if (result.status === 400) {
    res.status(400).json({ error: 'Bad request', message: result.message });
  } else {
    res.status(result.status).json({ error: 'Internal server error', message: result.message });
